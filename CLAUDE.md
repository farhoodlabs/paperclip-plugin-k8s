...
**Lease metadata fields the host reads.** `server/src/services/environment-execution-target.ts` reads `lease.metadata.remoteCwd` (where the agent's commands run) and `lease.metadata.shellCommand` (`bash` or `sh`). Other fields the plugin sets in metadata are not consumed by the host — they're informational. In particular, the host does **not** read `paperclipApiUrl` from lease metadata: for remote sandbox targets, the adapter unconditionally starts the callback bridge (`packages/adapter-utils/src/execution-target.ts:startAdapterExecutionTargetPaperclipBridge`) and `Object.assign`s its env (`PAPERCLIP_API_URL=http://127.0.0.1:<bridgePort>`, `PAPERCLIP_API_BRIDGE_MODE=queue_v1`, …) over the env you supply, so there is no "direct mode" toggle exposed to plugins.
